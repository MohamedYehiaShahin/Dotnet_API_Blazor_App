...
namespace API.Controllers
{
    ...
    public class AccountController(IAccount account) : ControllerBase
    {
        ...
        public async Task<ActionResult<GeneralResponse>> DeleteAccount(CreateAccountDTO model)
        {
            if (!ModelState.IsValid)
                return BadRequest("Model cannot be null");

            return Ok(await account.RemoveAccountAsync(model));
        }
        ...
        [HttpGet("identity/users-with-roles/list")]
